<template>
  ...
  >
    <div class="image-viewer-container">
      ...
      <div class="main-image-info">
        <div class="girl-info">
          ...
          <div v-else-if="!currImage.onService" class="info-value">该教师已下课</div>
          <div v-else-if="currImage.needCharge" class="info-value">
            您金币不足 点击&nbsp;<a style="cursor: pointer; color: #f90;" @click="recharge">购买</a>({{ currImage.price + '金币'}})&nbsp;获取联系方式
          </div>
          <div v-else class="info-value">
            <a style="cursor: pointer; color: #f90;" @click="buy">购买</a>({{ currImage.price + '金币'}})
          </div>
        </div>
    ...
    <div id="comment-area">
      <el-divider content-position="left">评论区<span
      ...
      <div class="comment-list" style="background-color: #101010; padding: 5px;">
        <div class="comment-input" style="margin-bottom: 30px;">
          <textarea
            class="comment-text"
            v-model="comment"
            placeholder="友善评论，不可泄漏本教师的联系或下载方式，若发现将做封号处理，多谢配合"
          ></textarea>
        ...
        <div  v-for="(item, index) in comments"
          ...
          <div class="comment-op" style="margin-top: 7px;">
            ...
            <span v-if="user === null || user.id !== item.commentatorId" style="color: #a3a2a2; font-size: 12px; cursor: pointer" @click="openReply(item.id, item.id, item.commentatorName)">回复</span>
            ...
            <div  v-for="(v, i) in item.replies"
              ...
              <div class="comment-op" style="margin-top: 7px;">
                ...
                <span v-if="user === null || user.id !== v.commentatorId" style="color: #a3a2a2; font-size: 12px; cursor: pointer" @click="openReply(item.id, v.id, v.commentatorName)">回复</span>
              </div>
            </div>
