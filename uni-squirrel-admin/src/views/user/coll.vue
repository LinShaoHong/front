    ...
    <el-card class="box-card">
      <div class="filter-container">
        ...
        <el-table v-loading="listLoading" :data="list" element-loading-text="正在查询中。。。" border fit highlight-current-row>
          ...
          <el-table-column align="center" label="收藏教师" prop="girlName" >
            <template slot-scope="scope">
              <span><a href="#" @click="toGirl(scope.row.girlId)">{{ scope.row.girlName }}</a></span>
            </template>
          </el-table-column>
          <el-table-column align="center" label="收藏时间" prop="updateTime">
            <template slot-scope="scope">
...
import { listCollects } from '@/api/collects'
...
export default class extends Vue {
  ...
  private async getList() {
    listCollects(this.listQuery).then(response => {
